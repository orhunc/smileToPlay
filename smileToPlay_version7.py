...
def detectSuprised(consecutive_suprises, section, pred_current):
    ...
    if 'Surprise' in pred_current or current_pred['Surprise'] >0.2:
...
def detectAngry(consecutive_angry, section):
    ...
    # Increase consecutive smile counter if smiling
    if 'Angry' in pred or current_pred['Angry'] >0.2:
        consecutive_angry += 1
    else:
        # Reset consecutive smile counter if not smiling
        consecutive_angry = 0
...
def video_processing():
    global noSmile, smileQuality, fakeness, section, last_time, end, last_section, last_change_time, consecutive_smiles, pred, emotion_timer, happiness_predictions, current_pred, previous_pred, consecutive_suprises, consecutive_angry, consecutive_happy, smile_sizes
